...
#ifndef BALLTZE_API__ENGINE__NETWORK_HPP
#define BALLTZE_API__ENGINE__NETWORK_HPP
...
namespace Balltze::Engine {
    enum MessageDeltaType : std::int8_t {
        MESSAGE_DELTA_TYPE_NONE = -1,
        MESSAGE_DELTA_TYPE_OBJECT_DELETION = 0,
        MESSAGE_DELTA_TYPE_PROJECTILE_UPDATE,
        MESSAGE_DELTA_TYPE_EQUIPMENT_UPDATE,
        MESSAGE_DELTA_TYPE_WEAPON_UPDATE,
        MESSAGE_DELTA_TYPE_BIPED_UPDATE,
        MESSAGE_DELTA_TYPE_VEHICLE_UPDATE,
        MESSAGE_DELTA_TYPE_HUD_ADD_ITEM,
        MESSAGE_DELTA_TYPE_PLAYER_CREATE,
        MESSAGE_DELTA_TYPE_PLAYER_SPAWN,
        MESSAGE_DELTA_TYPE_PLAYER_EXIT_VEHICLE,
        MESSAGE_DELTA_TYPE_PLAYER_SET_ACTION_RESULT,
        MESSAGE_DELTA_TYPE_PLAYER_EFFECT_START,
        MESSAGE_DELTA_TYPE_UNIT_KILL,
        MESSAGE_DELTA_TYPE_CLIENT_GAME_UPDATE,
        MESSAGE_DELTA_TYPE_PLAYER_HANDLE_POWERUP,
        MESSAGE_DELTA_TYPE_HUD_CHAT,
        MESSAGE_DELTA_TYPE_SLAYER_UPDATE,
        MESSAGE_DELTA_TYPE_CTF_UPDATE,
        MESSAGE_DELTA_TYPE_ODDBALL_UPDATE,
        MESSAGE_DELTA_TYPE_KING_UPDATE,
        MESSAGE_DELTA_TYPE_RACE_UPDATE,
        MESSAGE_DELTA_TYPE_PLAYER_SCORE_UPDATE,
        MESSAGE_DELTA_TYPE_GAME_ENGINE_CHANGE_MODE,
        MESSAGE_DELTA_TYPE_GAME_ENGINE_MAP_RESET,
        MESSAGE_DELTA_TYPE_MULTIPLAYER_HUD_MESSAGE,
        MESSAGE_DELTA_TYPE_MULTIPLAYER_SOUND,
        MESSAGE_DELTA_TYPE_TEAM_CHANGE,
        MESSAGE_DELTA_TYPE_UNIT_DROP_CURRENT_WEAPON,
        MESSAGE_DELTA_TYPE_VEHICLE_NEW,
        MESSAGE_DELTA_TYPE_BIPED_NEW,
        MESSAGE_DELTA_TYPE_PROJECTILE_NEW,
        MESSAGE_DELTA_TYPE_EQUIPMENT_NEW,
        MESSAGE_DELTA_TYPE_WEAPON_NEW,
        MESSAGE_DELTA_TYPE_GAME_SETTINGS_UPDATE,
        MESSAGE_DELTA_TYPE_PARAMETERS_PROTOCOL,
        MESSAGE_DELTA_TYPE_LOCAL_PLAYER_UPDATE,
        MESSAGE_DELTA_TYPE_LOCAL_PLAYER_VEHICLE_UPDATE,
        MESSAGE_DELTA_TYPE_REMOTE_PLAYER_ACTION_UPDATE,
        MESSAGE_DELTA_TYPE_SUPER_REMOTE_PLAYERS_ACTION_UPDATE,
        MESSAGE_DELTA_TYPE_REMOTE_PLAYER_POSITION_UPDATE,
        MESSAGE_DELTA_TYPE_REMOTE_PLAYER_VEHICLE_UPDATE,
        MESSAGE_DELTA_TYPE_REMOTE_PLAYER_TOTAL_UPDATE_BIPED,
        MESSAGE_DELTA_TYPE_REMOTE_PLAYER_TOTAL_UPDATE_VEHICLE,
        MESSAGE_DELTA_TYPE_WEAPON_START_RELOAD,
        MESSAGE_DELTA_TYPE_WEAPON_AMMO_PICKUP_MID_RELOAD,
        MESSAGE_DELTA_TYPE_WEAPON_FINISH_RELOAD,
        MESSAGE_DELTA_TYPE_WEAPON_CANCEL_RELOAD,
        MESSAGE_DELTA_TYPE_NETGAME_EQUIPMENT_NEW,
        MESSAGE_DELTA_TYPE_PROJECTILE_DETONATE,
        MESSAGE_DELTA_TYPE_ITEM_ACCELERATE,
        MESSAGE_DELTA_TYPE_DAMAGE_DEALT,
        MESSAGE_DELTA_TYPE_PROJECTILE_ATTACH,
        MESSAGE_DELTA_TYPE_CLIENT_TO_SERVER_PONG,
        MESSAGE_DELTA_TYPE_SUPER_PING_UPDATE,
        MESSAGE_DELTA_TYPE_SV_MOTD,
        MESSAGE_DELTA_TYPE_RCON_REQUEST,
        MESSAGE_DELTA_TYPE_RCON_RESPONSE,
        
        /** Balltze custom message */
        MESSAGE_DELTA_TYPE_BALLTZE_MESSAGE
    };

    enum class MessageDeltaMode : std::int32_t {
        STATELESS,
        INCREMENTAL
    };
    ...
    struct PACKED_ATTR MessageDeltaHeader {
        void *decoding_information;
        std::int32_t number;
    };

    struct PACKED_ATTR BalltzeMessage {
        std::uint16_t payload_size;
        std::byte payload[512];
    };
     ...
     */
    BALLTZE_API uint32_t mdp_encode_stateless_iterated(void *output_buffer, int32_t arg1, MessageDeltaType type, uint32_t arg3, void* message, uint32_t arg5, uint32_t arg6, uint32_t arg7);
     ...
     * @return The size of the encoded message (to be confirmed).
     */
    BALLTZE_API uint32_t mdp_encode_stateless_iterated(void *output_buffer, MessageDeltaType type, void *message);
     ...
     */
    BALLTZE_API bool mdp_decode_stateless_iterated(void *destination, MessageDeltaHeader *message_header);

    /**
     * Discards the iteration body of a message.
     * 
     * @param message_header The message header.
     */
    BALLTZE_API void mdp_discard_iteration_body(MessageDeltaHeader *message_header);

    /**
     * Sends a delta message to all players.
     * 
     ...
     */
    BALLTZE_API void send_delta_message_to_all(void* message, uint32_t message_size, bool ingame_only, bool write_to_local_connection, bool flush_queue, bool unbuffered, int32_t buffer_priority);

    /**
     * Sends a delta message to a player.
     * 
     ...
     * @param buffer_priority
     */
    BALLTZE_API void send_delta_message_to_player(int32_t player_id, void *message, uint32_t message_size, bool ingame_only, bool write_to_local_connection, bool flush_queue, bool unbuffered, int32_t buffer_priority);
}
