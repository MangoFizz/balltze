...
function PluginLoad() 
    Logger = Balltze.logger.createLogger("luastruct")

    Balltze.command.registerCommand("test", "debug", "Test LuaStruct functionality", nil, false, 0, 0, true, true, function()
        local mainMenuTagHandle = Ringworld.tag.lookupTag("ui\\shell\\main_menu\\main_menu", "ui_widget_definition")
        if not mainMenuTagHandle then
            Logger:error("Failed to find main menu tag")
            return false
        end

        local mainMenuTagEntry = Ringworld.tag.getTagEntry(mainMenuTagHandle)
        if not mainMenuTagEntry then
            Logger:error("Failed to get main menu tag entry")
            return false
        end

        local mainMenuTag = Ringworld.tag.getTagData(mainMenuTagHandle, "ui_widget_definition")
