...
namespace Balltze::LegacyApi::Plugins {
    ...
    bool reinit_plugins_on_next_tick;
    ...
    static void init_plugins() noexcept {
        logger.info("Initializing legacy plugins...");
        Balltze::Plugins::init_plugins_path();
        auto plugins_path = Balltze::Plugins::get_plugins_path();
    ...
    static void plugins_tick(TickEvent const &context) noexcept {
        ...
        if(reinit_plugins_on_next_tick) {
            logger.info("Reloading legacy plugins...");
...
namespace Balltze {
    std::filesystem::path get_plugin_path(PluginHandle handle) {
        auto *plugin = LegacyApi::Plugins::get_dll_plugin(reinterpret_cast<HMODULE>(handle));
