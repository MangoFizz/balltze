...
#include <balltze/legacy_api/engine/tag_definitions/ui_widget_definition.hpp>
...
namespace Balltze::LegacyApi::Engine {
    ...
    Resolution get_bitmap_sprite_resolution(LegacyApi::Engine::TagDefinitions::Bitmap *bitmap, std::size_t sequence_index, std::size_t sprite_index) {
    ...
    void draw_hud_message_sprite(LegacyApi::Engine::TagDefinitions::Bitmap *bitmap, std::size_t sequence_index, std::size_t sprite_index, Point2DInt position, ColorARGBInt color) {
        if(sequence_index >= bitmap->bitmap_group_sequence.count) {
            throw std::runtime_error("Invalid sequence index");
        }
        auto &sequence = bitmap->bitmap_group_sequence.elements[sequence_index];
